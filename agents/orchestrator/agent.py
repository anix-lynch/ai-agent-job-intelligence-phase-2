"""Orchestrator: ReAct-style job matching agent."""
...
class JobMatchingAgent:
    """Job matching agent with ReAct-style reasoning."""
    ...
    def run(self, query: str, jobs_df: pd.DataFrame) -> str:
        ...
        for location_key, location_patterns in location_keywords.items():
            if location_key in query_lower:
                ...
                location_mask = filtered["location"].str.contains(
                    "|".join(location_patterns), case=False, na=False
                )
                filtered = filtered[location_mask]
                filters_applied.append(f"location: {location_found}")
                self.reasoning_trace.append(
                    f"👁️ Observation: User wants jobs in {location_found}"
                )
                break
        if "200k" in query_lower or "$200" in query_lower or "200000" in query_lower:
            self.reasoning_trace.append(
                "👁️ Observation: User wants high-salary roles (200K+)"
            )
        ...
        if "senior" in query_lower:
            self.reasoning_trace.append(
                "👁️ Observation: User wants senior-level positions"
            )
            filtered = filtered[
                filtered["title"].str.contains(
                    "Senior|Staff|Principal|Lead", case=False, na=False
                )
            ]
        ...
        if "ml" in query_lower or "machine learning" in query_lower:
            self.reasoning_trace.append(
                "👁️ Observation: User interested in ML engineering roles"
            )
            filtered = filtered[
                filtered["title"].str.contains("ML|Machine Learning", case=False, na=False)
            ]
            ...
            filtered = filtered[
                filtered["title"].str.contains(
                    "AI|Artificial Intelligence", case=False, na=False
                )
            ]
        ...
        self.reasoning_trace.append(
            f"🤔 Thought: Found {len(filtered)} {role_type} positions ({filter_summary})"
        )
        ...
        for idx, job in filtered.iterrows():
            salary_range = (
                ...
                if pd.notna(job.get("salary_min"))
                else "Competitive"
            )
        ...
        self.reasoning_trace.append(
            f"📊 Final Answer: Presented {len(filtered)} curated opportunities"
        )
        return response
